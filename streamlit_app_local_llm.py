...
#------------------------------------------------------------------------------------
#-----------------------------------------------------------------------------------
#    FOR USING LOCAL LLAM THROUGH OLLAMA 
...
#!/usr/bin/env python3
"""
Task 9: Streamlit Research AI Assistant (Ollama Version)
Interactive web interface using LangGraph and Llama 3.1
"""
...
from typing import TypedDict, List, Literal
from langgraph.graph import StateGraph, START, END
from langchain_ollama import ChatOllama  # Replaced langchain_openai
from ddgs import DDGS
from datetime import datetime
import json
...
# Page config
st.set_page_config(
    page_title="Ollama Research Assistant",
    page_icon="🤖",
    layout="wide"
)
...
@st.cache_resource
def get_llm():
    # Using llama3.1:latest running locally via Ollama
    return ChatOllama(
...
def input_processor_node(state: ResearchState):
    ...
    prompt = f"Given this research topic: '{state['topic']}', suggest a more specific research focus. Return only the one-line focus."
...
def search_tool_node(state: ResearchState):
    ...
    # Process only new questions to save time/resources
    new_questions = state["research_questions"][-3:]
...
def analyzer_node(state: ResearchState):
    ...
    all_findings = state.get("key_findings", []) + findings
    quality = min(len(all_findings) * 0.1, 1.0)
...
def main():
    ...
    st.caption("Powered by LangGraph & Ollama (Llama 3.1)")
    
    # Sidebar
    with st.sidebar:
        st.header("⚙️ Settings")
        max_iterations = st.slider("Max Iterations", 1, 5, 2)
        st.info("Running locally on Llama 3.1:latest")

    # Research Input
    topic = st.text_input("What would you like me to research?", placeholder="e.g. History of AI")
    
    if st.button("🚀 Start Research", type="primary") and topic:
        initial_state = {
            "topic": topic,
            "research_questions": [], "search_queries": [], "search_results": [],
            "key_findings": [], "iteration": 0, "max_iterations": max_iterations,
            "quality_score": 0.0, "final_report": "", "status": "initialized",
            "current_node": ""
        }
        
        with st.status("Researcher is thinking...", expanded=True) as status:
            assistant = build_workflow()
            try:
                result = assistant.invoke(initial_state)
                status.update(label="Research Complete!", state="complete")
