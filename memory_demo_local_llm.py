...
#!/usr/bin/env python3
"""
Task 8: Memory & State Accumulation Demo (Ollama Version)
Shows how state persists and accumulates across nodes
...
print("\n💾 MEMORY & STATE ACCUMULATION DEMO (Llama3.1 Local)")
print("=" * 50)
...
def generate_questions_node(state: MemoryState):
    ...
    prompt = f"""
Generate 3 important research questions about:
{state['topic']}
...
"""
    ...
    response = llm.invoke(prompt)
    questions = response.content.strip().split("\n")
    questions = [q.strip("- ").strip() for q in questions if q.strip()][:3]
...
def search_node(state: MemoryState):
    ...
    for question in state["questions"]:
        result = (
            f"Result for '{question[:40]}...': "
            f"Found relevant structured information about {state['topic']}."
        )
        search_results.append(result)
        print(f"   → Searched: {question[:50]}...")
...
def extract_key_points_node(state: MemoryState):
    ...
    prompt = f"""
Extract 5 key insights about {state['topic']} from the following:
...
"""
    ...
    response = llm.invoke(prompt)
    new_points = response.content.strip().split("\n")
    new_points = [p.strip("- ").strip() for p in new_points if p.strip()][:5]
...
def build_knowledge_base_node(state: MemoryState):
    print(f"\n📚 Build Knowledge Base Node")
    print(f"   Synthesizing accumulated state...")

    prompt = f"""
Create a comprehensive structured summary about:
...
Provide a well-organized knowledge summary.
"""

    response = llm.invoke(prompt)
    knowledge_base = response.content.strip()
...
def display_memory_state(state: MemoryState):
    ...
    print(f"\n📚 Knowledge Base Preview:")
    print(state['knowledge_base'][:400] + "...")
...
print("=" * 50)
print("""
1. State persists across all nodes
2. Lists accumulate data over time
3. Nodes read and extend existing state
4. State acts as workflow memory
5. Final nodes synthesize accumulated knowledge
""")
