...
#    ---------------------------------------------------------------------------------------------------------------------------------------------------------
#    ---------------------------------------------------------------------------------------------------------------------------------------------------------
#    FOR USING LOCAL LLM BY OLLAMA
#    PRACTISE MODEL: llama3.1:latest
#    ---------------------------------------------------------------------------------------------------------------------------------------------------------
#    ---------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    #!/usr/bin/env python3
"""
Task 6: Loops & Iterations Demo (Ollama Version)
Shows how to create iterative refinement loops in LangGraph
...
print("\n🔄 LOOPS & ITERATIONS DEMO (Ollama - Llama3.1)")
print("=" * 50)

# Initialize LOCAL LLM (no API key required)
llm = ChatOllama(
...
def search_node(state: ResearchState):
    ...
    # Simulate improving search results
    if iteration == 0:
        results = f"Basic results for: {query}"
        print("   → Found basic information")
    elif iteration == 1:
        results = f"Detailed results for: {query} with more context"
        print("   → Found detailed information")
    else:
        results = f"Comprehensive results for: {query} with citations and examples"
        print("   → Found comprehensive information")
...
def summarize_node(state: ResearchState):
    """Generate final answer using Llama 3.1"""
    print("\n✨ Summarize Node")

    prompt = f"""
You are a helpful research assistant.

Based on these search results, provide a concise and clear answer.
...
"""
    ...
    print("   → Generated final answer using Llama 3.1")
...
for query in test_queries:
    ...
    print(f"   Total iterations: {result['iteration']}")
    print(f"   Final quality: {result['quality_score']:.1f}")
    print(f"   Answer preview: {result['final_answer'][:120]}...")
